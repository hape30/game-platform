...
class MysticalMenu(GameScene):
    def __init__(self, goto_scene):
        self._goto_scene = goto_scene
        GameScene.__init__(self)

        self.bg = pygame.image.load(os.path.join(settings.img_folder, "mystical_bg.jpg"))
        self.bg = pygame.transform.scale(self.bg, (settings.SCREEN_WIDTH, settings.SCREEN_HEIGHT))

        self.title_font = Font(None, 80)
        self.button_font = Font(None, 40)

        self.title_text = self.title_font.render("Древний лес", True, (255, 255, 255))
        self.title_rect = self.title_text.get_rect(center=(settings.SCREEN_WIDTH // 2, settings.SCREEN_HEIGHT // 4))

        self.start_button = Button((settings.SCREEN_WIDTH // 2, settings.SCREEN_HEIGHT // 2), "Начать", self.button_font, self.start_game)
        self.quit_button = Button((settings.SCREEN_WIDTH // 2, settings.SCREEN_HEIGHT * 3 // 4), "Выход", self.button_font, self.quit_game)

    def start_game(self):
        self._goto_scene("game")
